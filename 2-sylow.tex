...
\begin{defn}
    ...
    In this case we will also say that\/ $G$ \textsl{acts on\/} $X$. There is also the notion of a \textsl{right} action in which\/ $G$ acts from the right on $X$.
\end{defn}
...
\begin{rem}\label{actions-are-group-morphisms}
    ...
    $$
        1\cdot x=\sigma_1(x)=\id_X(x)=x\quad\text{\upshape and}\quad
...
\begin{defn}
    ...
    $$
        \omega\cdot x=x\quad\textrm{\upshape for all }x\in X.
    $$
...
\begin{enumerate}[\upshape 1.]
    ...
    \item Take $X=2^G$, the power set of $G$. We can define here two left actions $A\mapsto A^\omega$ and $A\mapsto\omega A$ and one right action $A\mapsto A\omega$. Note that these actions remain well-defined if we restrict $X$ to certain subsets that are preserved by conjugation and left (resp.~right) multiplication. For instance, $X$ could be {\upshape(a)}~the set of all subgroups of\/ $G$, {\upshape(b)}~the set of parts of a given cardinality or {\upshape(c)}~the set $\lco GH$ of left cosets of a given subgroup~$H$.
\end{enumerate}
\end{xmpls}
...
\begin{rem}\label{basic-stabilizers}
    ...
    \begin{enumerate}[\upshape i)]
        \item \textrm{\upshape regular action:} $G_\omega=\gen1$, $\omega\in G$.
        \item \textrm{\upshape conjugation:} $G_\omega=C_G(\omega)$, $\omega\in G$.
        \item \textrm{\upshape conjugation on $2^G$:} $G_X=N_G(X)$, $X\in 2^G$.
    \end{enumerate}
\end{rem}
...
\begin{proof}
\begin{align*}
    \zeta\in(G_x)^\omega &\iff \zeta=\omega\nu\omega^{-1},\quad\textrm{\upshape for some }\nu\in G_x\\
...
\begin{proof}     
    In fact, the kernel of $\sigma$ is the intersection of all stabilizer groups, which is precisely the core as shown in {\upshape Lemma~\ref{core}}.  \end{proof}
...
\begin{enumerate}[\upshape a)]
    \item By {\upshape Lemma~\ref{core=ker}} $\Core_G(H)$ is normal. It is contained in $H$ because $1\in G$ and $H^1=H$.
    
    If $N\normal G$ and $N\subseteq H$, then $N=N^x\subseteq H^x$ for all $x\in G$ and therefore $N\subseteq\Core_G(H)$.

    \item According to {\upshape Lemma~\ref{core=ker}} $G/\Core_G(H)$ is isomorphic to a subgroup of\/ $\Sym(\lco GH)$. Then the conclusion follows because $n=|G:H|$.
\end{enumerate}
\end{proof}

\begin{cor}\label{n!-theorem}{\upshape[$n!$ theorem]}
...
\begin{defn}
    A group is \textsl{simple} if it doesn't have any nontrivial normal subgroup.  {\upshape[cf.~Exercise~\ref{exr:simple-group-mention}]}
\end{defn}

\begin{cor}\label{order-divide-n!}
...
\begin{enumerate}[\upshape 1)]
    ...
    \item For the case\/ $X=G$ we have the conjugation map $(x,\omega)\mapsto x\omega x^{-1}$. The orbit of an element $\omega$ is known as the \textsl{conjugacy class} of\/ $\omega$ and denoted by~$[\omega]$, i.e.,
    $$
        [\omega] = {\cal O}_\omega = \set{x\omega x^{-1}\mid x\in G}.
    $$
    By\/ {\upshape Proposition \ref{normal-closure}}, the \textsl{normal closure} of the subgroup $\gen\omega$ is
    $$
        \gen\omega^*=\gen{[\omega]}.
    $$
\end{enumerate}
...
\begin{thm}{\upshape[The Fundamental Counting Principle]}\label{fundamental-counting-principle}
...
\begin{cor}\label{p-groups-have-center}
    ...
    \textrm{\upshape[See also Theorem~\ref{nontrivial-center}]}
\end{cor}
...
\end{align*}
 ...
 $$
 Now, given that $[\omega]=G$ if, and only if, $\omega=1$, and in such a case $G$ would be trivial, the previous corollary implies that every $\abs{[\omega_i]}$ is a positive power of $p$. In particular, $\abs{[\omega_i]}\equiv0$ modulo $p$ and so
 $$
...
\begin{cor}\label{p-squared-is-abelian}
    If\/ $|G|=p^2$ then $G$ is abelian. \textrm{\upshape [cf.~Problem~\ref{problem-1.D.9}]}
\end{cor}
...
\begin{solution} See Corollaries \ref{index-inequality} and \ref{coprime-indices}.  \end{solution}
...
$$
    H^x=H^{kh}=(H^h)^k=H^k.
$$
This means that for the product $H^xK^y$ we may assume that $x\in K$ and $y\in H$. Put\/ $\omega=x^{-1}y$. Thus, given $z\in G$, we can write it as $z = x(x^{-1}zy)y^{-1}$. Since $x^{-1}zy\in G=HK$, we obtain $z\in xHKy^{-1}$. Now, using that $KH=HK$, we can pick $a\in H$ and $b\in K$ such that $\omega=ab$. Then, $H\omega K=HabK=HK$. In consequence,
$$
...
\begin{probl}
    ...
    \textrm{\upshape Hint. Show that if $G_\alpha G_\beta=G$ for some $\alpha \in S$ and $\beta \in T$, then in fact, this holds for all $a \in S$ and $b \in T$.}
\end{probl}
    
\begin{proof} 
\begin{description}
    
\item[\upshape\textit{if\/ } part:] First observe that $G_a^\omega=G_{\omega^{-1}\cdot a}$. Therefore, by the previous problem, $G_{x\cdot\alpha}G_{y\cdot\beta}=G$ for all $x,y\in G$, which, in view of the hypothesis, implies that $G_sG_t=G$ for every $(s,t)\in S\times T$.
...
\item[\upshape\textit{only if\/}:] Take $x\in G$ and a pair $(s,t)\in S\times T$. By transitivity there exists $y\in G$ such that
$$
...
\begin{probl}\label{problem-1.A.7}
    ...
    \textrm{\upshape Hint. Let $\chi$ be the permutation character associated with the action of\/ $G$ on the left cosets of $H$. Then $\sum_{x \in G}\chi(x) = |G|$. Show that $\sum_{z \in H}\chi(z) > 2\abs H$. Use this information to get an estimate on the number of elements of\/ $G$ where~$\chi$ vanishes.}
\end{probl}
...
\begin{solution}
    ...
    $$
    Therefore, the number $n$ of such orbits is at least $2$. Let $\chi^H\colon H\to\N_0$ be the permutation character associated with the action of $H$ on $S$. We have
    $$
...
\begin{enumerate}[\upshape a)]
    ...
    $$
        (\Z_p)_{(x_1,\dots,x_p)}=\begin{cases}
            \Z_p&\text{if } x_1=\cdots=x_p,\\
            \set0&\text{otherwise}.
        \end{cases} 
    $$
    To see this assume that $k\ne0$ is in the stabilizer. Take two indices $i<j$ and let's show that $x_i=x_j$. Define $q\in\Z_p$ by $qk\equiv j-i\pmod p$. Given that~$k$ satisfies $x_\cdot= x_{\cdot+k}$, we have
    $$
    ...
    $$
        |(\Z_p)_{(x_1,\dots,x_p)}|=\begin{cases}
            p&\text{if } x_1=\cdots=x_p=x,\; x^p=1,\\
            1&\text{otherwise}.
        \end{cases} 
    $$
...
\begin{solution} By the previous problem, there exists at least one element $x\in G$ with $\ord(x)=p$. Thus, $\gen x$ is a subgroup of order $p$.
...
\begin{prop}
    ...
    $$
        \ker(\sigma)=Z(G)\quad\text{\upshape and}\quad G/Z(G)\cong\Inn(G).
    $$
\end{prop}
...
\begin{proof} By the proposition there exists $x\in G$ such that $\Inn(G)=\gen{\sigma_x}$. Take $y\in G$. Then $\sigma_y=(\sigma_x)^n=\sigma_{x^n}$, i.e., $y=x^nz$ for some $z\in Z(G)$. Therefore, $x\leftrightarrow y$ because $x\leftrightarrow x^n$ and $x\leftrightarrow z$.  \end{proof}

\begin{rem} Let $G$ be a group and $H$ a subgroup. Then
\begin{align*}
    H\normal G &\iff \varsigma(H)=H\quad\text{\upshape for all } \varsigma\in\Inn(G)\\
    H\ch G &\iff \theta(H)=H\quad\text{\upshape for all } \theta\in\Aut(G).
\end{align*}
\end{rem}
...
\begin{exr}
    Let\/ $G$ be finite with\/ $Z(G) = \gen1$, and set\/ $\Gamma = \Aut(G)$ and\/ $\Lambda := \text{\upshape Inn}(G)$.
    \begin{enumerate}[\upshape a)]
...
\begin{enumerate}[\upshape a)]
    ...
    \item The RHS is clearly contained in the LHS. Moreover, the RHS is a group because it is the image of $\ct_x$. Therefore, to prove the other inclusion it suffices to show that $[x^i,a]$ is an element of $\im(\ct_x)$ for every $i\in\Z$. But
    \begin{align*}
...
\begin{exr}
    Let\/ $A$ and\/ $x$ be as in\/ \textrm{\upshape Exercise~\ref{exercise-1.5.1}}. Suppose that for all\/ $a\in A$ we have $G = C_G(xa)A$. Then\/ $[G,A]=[\gen x,A]$.

    \textrm{\upshape Hint [l.c.]: Given $y\in G$ and $a\in A$, write $y=zb$ with $z\leftrightarrow x$ and then $z=wc^{-1}$ with $w\leftrightarrow xa$.}
\end{exr}
...
\begin{exr}\label{exercise-1.5.3}
    ...
    \begin{enumerate}[\upshape a)]
      ...
      \item $G' \subseteq Z(G)$.
      \item \textrm{\upshape [Knoche]} $|G'|\le p$.
    \end{enumerate}
\end{exr}
...
\begin{exr} {\upshape [Ito]}
    Let\/ $G = AB$, where\/ $A$ and\/ $B$ are abelian subgroups of\/ $G$. Then\/ $G'$ is abelian.

    \textrm{\upshape Hint [l.c.]:} $[A,B]$ is abelian and $G'=[A,B]$.
\end{exr}
...
$$
As a result, $[A,B]$ is abelian. By Proposition~\ref{commutator-props}, $[A,B]\normal G$. Consider $\bar G=G/[A,B]$. Given $a\in A$ and $b\in B$, we have $ab=[a,b]ba$, i.e., $\bar a\bar b=\bar b\bar a$, which shows that $\bar G$ is abelian. By the lemma of Exercise~\ref{exercise-1.5.3}, $G'\subseteq[A,B]$. Since $[A,B]$ is clearly contained in $G'$, we deduce that $G'=[A,B]$.  \end{solution}

\begin{exr} \textrm{\upshape[Burnside]}
...
\begin{defns}
    A \textsl{$p$-group} is a finite group whose order is a power of a prime number $p$. A \textsl{Sylow $p$-subgroup} of a finite group $G$ is a $p$-subgroup $P$ whose order is as large as allowed by Lagrange's theorem, that is, $\abs P=p^e$ with $e\ge0$ and $p\perp m$, where $\abs G = p^em$. The\/ \textrm{\upshape Sylow-E Theorem~\ref{sylow-e}} states that Sylow subgroups always exist.
\end{defns}
...
\begin{thm}\label{sylow-e} {\upshape[Sylow-E]}
...
\begin{proof} Suppose by contradiction that $\spec\abs H\not\subseteq\pi$. Pick $p\in\spec\abs H\setminus\pi$. By Cauchy's Theorem {\upshape [cf.~Problem~\ref{cauchy}]}, there exists $y\in H$ with $\ord(y)=p$.  \end{proof}
...
\begin{enumerate}[\upshape a)]
    ...
    $$
    where $p^s=\abs{Q\cap P}$. It is now clear that the equivalence $QP \le G \iff Q \subseteq P$ follows directly from the definitions.
...
\begin{probl}\label{problem-1.B.3}
    ...
    \textrm{\upshape{\bf Note.} For a generalization of this result see Problem~\ref{problem-1.C.1}.}
\end{probl}
...
\begin{probl}\label{sylow-e-2}
    ...
    \textrm{\upshape Hint. Use Problem~\ref{problem-1.A.10} and consider the group $N_G(P)/P$.}
\end{probl}
...
\begin{enumerate}[\upshape a)]
    ...
    $$
        |G:\ker\varphi|=p^em\quad\text{and}\quad
...
\begin{probl}\label{problem-1.B.6}
    ...
    \textrm{\upshape\textbf{Note.} In particular, $K$ has a Hall $\pi$-subgroup, namely $K\cap H$, if either $H$ or $K$ is normal in $G$ since in that case, $KH$ is guaranteed to be a subgroup.}
\end{probl}
...
\begin{probl}\label{problem-1.D.7} Let $G$ be a finite group and $\pi$ any set of primes.
    ...
    \item Show that the subgroup\/ $N$ of part\/~{\upshape a)} is contained in every Hall\/~$\pi$-subgroup of\/~$G$.

    \item Assuming that $G$ has a Hall\/ $\pi$-subgroup, show that $N$ is exactly the intersection of all of the Hall\/ $\pi$-subgroups of\/~$G$.
    \end{enumerate}

    \textrm{\upshape {\bf Note.} The subgroup $N$ of this problem is denoted $O_\pi(G)$. Because of the uniqueness in a), it follows that this subgroup is characteristic in $G$. If $p$ is a prime number, then $O_{\set p}(G) = O_p(G)$ [cf.~Problem~\ref{problem-1.B.2}].}
\end{probl}
...
\begin{enumerate}[\upshape a)]
    ...
    \item From part b) we know that $N$ is contained in the intersection of all Hall $\pi$-subgroups.
...
\begin{probl}\label{problem-1.B.8}
    ...
    \item Show that the subgroup\/ $N$ of part\/ {\upshape a)} is generated by the set of all elements of\/~$G$ that have order not divisible by any prime in $\pi$.
    \end{enumerate}

    \textrm{\upshape\textbf{Note.} The characteristic subgroup $N$ of this problem is denoted $O^\pi(G)$.}
    
    \textrm{\upshape {\bf Note.} For an MSE solution see~\href{https://math.stackexchange.com/q/688412/269050}{\it A problem about $\pi$-groups}.}
\end{probl}
...
\begin{thm}\label{sylow-c} {\upshape[Sylow-C]}
...
\begin{rem}
    The theorem provides an alternative proof of part\/ {\upshape b)} of {\upshape Problem \ref{problem-1.B.1}}.
\end{rem}
...
\begin{rem}
    The reader should understand the previous proof under the light of\/ {\upshape Lemma~\ref{core}}, {\upshape Definition~\ref{Core_G}}, {\upshape Remark~\ref{G-to-Sym(X)}} and\/ {\upshape Lemma~\ref{core=ker}}.
\end{rem}

\begin{cor}\label{frattini-argument} {\upshape[Frattini Argument]}
...
\begin{rem}\label{frattini-argument-2}
    The\/ \textrm{\upshape Frattini Argument} admits a more general statement:
    ...
    \textrm{\small\upshape Take $x\in G$ and $\alpha\in X$. By the transitivity of $N$ there exists $y\in N$ with $x\cdot\alpha=y\cdot\alpha$. In consequence, $y^{-1}x\in G_\alpha$, i.e., $x\in NG_\alpha$.}
\end{rem}

\begin{thm}\label{sylow-d} {\upshape[Sylow-D]}
...
\begin{rem}\label{p'-part}
    It follows from {\upshape Corollary \ref{n_p-is-index}} that
    $$
...
\begin{xmpl}
    ...
    \begin{enumerate}[$1$.]
        \item $n_7\mid2^6$ $\leftarrow$ {\upshape Remark \ref{p'-part}}.
        \item $n_7\equiv1\pmod7$ $\leftarrow$ {\upshape Corollary \ref{p|n_p-1}}.
    ...
    \begin{enumerate}
        \item[$5$.] $P\cap Q\normal P$ $\leftarrow$ {\upshape Problem~\ref{problem-1.A.1} for $G=P$}.
        \item[$6$.] $P\cap Q\normal Q$ $\leftarrow$ {\upshape Problem~\ref{problem-1.A.1} for $G=Q$}.
        \item[$7$.] $P\cup Q\subseteq H$ $\leftarrow$ {\upshape $5$ and $6$}.
        \item[$8$.] $n_7(H)>1$ $\leftarrow$ $P,Q\in\Syl_7(H)$.
        \item[$9$.] $n_7(H)\ge8$ $\leftarrow$ {\upshape $8$ and Corollary \ref{p|n_p-1}}.
        \item[$10$.] $2^3\mid\abs H$ $\leftarrow$ {\upshape $\spec\abs H=\set 2$ and $9$}. 
        \item[$11$.] $\abs{G:H}\le2^3$ $\leftarrow$ {\upshape $7^3=\abs P\mid\abs H$ and $10$}.
        \item[$12$.] $\abs G\nmid\abs{G:H}!$ $\leftarrow$ {\upshape $7!<\abs G$ and $\abs G\nmid8!$}.
        \item[$13$.] $H\ne G\implies G$ is not simple $\leftarrow$ {\upshape $n!$~theorem~\ref{n!-theorem} and $12$}.
    \end{enumerate}
    \textsc{Claim: }$G$ is not simple.
    \begin{enumerate}
        \item[$14$.] If\/ $n_7=1$ then\/ $\Syl_7(G)=\set P$ with $P$ normal $\leftarrow$ $P=O_7(G)$.
        \item[$15$.] If\/ $n_7\ne1$, $G$ is not simple $\leftarrow$ {\upshape $13$ or $\gen1\varsubsetneq P\cap Q\normal G$}.
    \end{enumerate}
...
\begin{probl}\label{problem-1.C.1}
    ...
    \textrm{\upshape {\bf Note}. This generalizes Problem \ref{problem-1.B.3}}.
\end{probl}
...
\begin{enumerate}[\upshape a)]
    ...
    Since $H\cap Q$ is a $p$-subgroup of $H$, it must be contained in some Sylow $p$-subgroup $R\in\Syl_p(H)$. Then $P\subseteq H\cap Q\subseteq R$, which implies $P=R$ because both groups have order $p^d$.
...
\begin{probl}\label{problem-1.C.3}
    ...
    \begin{enumerate}[\upshape a)]
        ...
        \textrm{\upshape Hint. Let a Sylow $p$-subgroup of $G$ act on $X$.}
    \end{enumerate}

    \textrm{\upshape\textbf{Note.} In other words, if $p\mid|G|$, the number of elements whose order is a positive power of $p$ is congruent to $-1$ modulo $p$}
\end{probl}
...
\begin{probl}
    ...
    \textrm{\upshape Hint: Analyze separately the four possibilities for $n_2(G)$}.
\end{probl}
...
\begin{enumerate}[1.]
    ...
    \item $n_5\in\set{1, 6}$ $\leftarrow$ $n_5\equiv1\pmod5$ and $n_5\mid 2^3\cdot 3$
    \item Case $n_2=1$:
...
\begin{probl}\label{problem-1.C.5}
    ...
    \textrm{\upshape Hint. Count the elements of order $p$ in $G$.}
\end{probl}
...
$$
    \sigma=\zeta_1\cdots\zeta_k,
$$
where $\ord(\zeta_i)=p$. In particular, $pk\le p+1$ and so $k=1$. This means that the permutations of order $p$ are all the cyclic permutations of $p$ symbols, which account for a total of $(p+1)(p-1)!$ cycles: for every fixed $k\in\nset{p+1}$, consider the $(p-1)!$ cyclic permutations of the remaining $p$ integers.
...
\begin{probl}
    ...
    \begin{enumerate}[\upshape a)]
        ...
        \textrm{\upshape Hint. First choose $Q\in\Syl_p(G)$ and $x\in G$ such that $Q\cap H\in\Syl_p(H)$ and $Q^x\cap K\in\Syl_p(K)$. Write $x^{-1}y=w$, with $y\in H$ and $w\in K$.}

        \item If $P$ is as in\/ {\upshape a)}, then $P=(P\cap H)(P\cap K)$.
    \end{enumerate}
\end{probl}
...
\begin{probl}\label{problem-1.C.7}
    ...
    \textrm{\upshape Hint. Otherwise, let $M$ be a maximal subgroup of $G$ containing $N_G(P)$, where $P\in\Syl_p(G)$. Compare $n_{p}(M)$ and $n_p(G)$.}
\end{probl}
...
\begin{thm}\label{nontrivial-center}
    ...
    \textrm{\upshape[See also Corollaries \ref{p-groups-have-center} and \ref{normal-nilpotent-center}].}
\end{thm}
...
\begin{rem}\label{nontrivial-nilpotent-center}
    If\/ $G$ is nilpotent and nontrivial, the first nontrivial subgroup of a central series is contained in the center\/ $Z(G)$. In particular, $Z(G)\ne\gen1$.
\end{rem}
...
\begin{proof}${}$
\begin{enumerate}[\upshape a)]
    \item $\Rightarrow$ b) This follows from Corollary~\ref{nilpotent-subgroups-and-quotients} and the fact that the first nontrivial term of a central series is contained in the center.
...
\begin{probl}
    ...
    \textrm{\upshape Hint. If $y$ and $y^z$ lie in $H$, where $z \in N$, observe that $y^{-1}y^z \in N$.}
\end{probl}
...
\begin{description}[leftmargin=!,labelwidth=\widthof{\it if:}]
    \item[\textrm{\upshape{\it if\/} part:}] Take $y^x\in H\cap H^x$ with $y
    ...
    \item[\textrm{\upshape{\it only if\/}:}]  Take $y\in H\setminus\set1$ and $z\in C_N(y)$. Then $y=y^z\in H\cap H^z$. Since $H$ is a Frobenius complement, we deduce $z\notin G\setminus H$, i.e., $z\in H$. Thus, $z\in N\cap H=\gen1$.
\end{description}
\end{solution}
...
\begin{probl}
    ...
    \textrm{\upshape Hint. Observe that the hypothesis is satisfied by $H \cap H^x$ for $x \in G$. If this intersection is nontrivial, consider a nontrivial Sylow subgroup $Q \subseteq H \cap H^x$ and show that $Q$ and $Q^{x^{-1}}$ are conjugate in $H$.}
\end{probl}
...
\begin{description}%[labelwidth=\widthof{\it if:}]
    \item[\upshape{\it if\/} part:] This is trivial. If $H\subseteq G$ is a subgroup with index $\abs{G:H}=p$ prime and $H\subseteq K\subseteq G$, then $\abs{G:K}\mid p$. So $K=G$, for $\abs{G:K}=1$, or $K=H$, for $\abs{G:K}=p$.
    
    \item[\upshape{\it only if\/}:] Let $M$ be a (proper) maximal subgroup of the nilpotent group $G$. By Theorem~\ref{nilpotent-equivalences} $M$ is normal and by Theorem~\ref{nilpotent-center-series}, $\bar G=G/M$ has a nontrivial center. Pick $\bar z\in Z(\bar G)\setminus\gen1$. The cyclic group $\gen{\bar z}$ is normal and since $z\notin M$, it must be $\gen zM=G$, i.e., $\bar G=\gen{\bar z}$. Finally, $\ord(\bar z)$ has to be prime, otherwise, there would be some subgroup between $M$ and $G$.
\end{description}
\end{solution}
...
\begin{probl}
    ...
    \textrm{\upshape Hint. If $A\varsubsetneq C_G(A)$, apply Lemma~\ref{subgroup-of-index-p}.}
\end{probl}
...
Therefore, the induced morphism $\bar\sigma\colon G/A\to\Sym(A)$ is a monomorphism. Since $\sigma_x(1)=1$,
 $\im(\bar\sigma)$ is contained in the subgroup $F$ of $\Sym(A)$ of bijections that fix~$1$. Clearly, $\abs F=(\abs A-1)!$ and $\im(\bar\sigma)$ is a subgroup of $F$, isomorphic to $G/A$. Thus, the order of $G/A$ must divide $(\abs A-1)!$  \end{solution}

 \begin{probl}
     Let\/ $n$ be the maximum of the orders of the abelian subgroups of a finite group\/ $G$. Show that\/ $|G|$ divides\/ $n!$.
    
    \textrm{\upshape Hint. Show that for each prime $p$, the order of a Sylow $p$-subgroup of $G$ divides~$n$!}

    \textrm{\upshape {\bf Note.} There exist infinite groups in which the abelian subgroups have bounded order, so finiteness is essential here.}
 \end{probl}
...
$$
for $i>1$ because $Z\subseteq N_i$ for those indices. Since $Z\subseteq Z(G)$, we can extend the sequence to the left with $Z\subseteq N_1$ and get a normal series for $G$. The conclusion now follows from Proposition~\ref{ad-nilpotent-test} because $\ct_x(N_1)\subseteq Z\subseteq Z(G)$.  \end{solution}

\begin{probl}
    Show that the Frattini subgroup\/ $\Phi(G)$ of a finite group\/ $G$ is nilpotent.

    \textrm{\upshape Hint. Apply the Frattini argument. The proof here is somewhat similar to the proof that (c) implies (d) in Theorem~\ref{nilpotent-equivalences}}.
    
    \textrm{\upshape {\bf Note.} This problem shows that $\Phi(G) \subseteq F(G)$.}
\end{probl}
...
\begin{probl}\label{problem-1.D.15}
    ...
    \textrm{\upshape{\bf Note.} This generalizes the previous problem by setting $N = \Phi(G)$. Note that this problem proves that $F(G/\Phi(G)) = F(G)/\Phi(G)$.}
\end{probl}

\begin{solution}
    Without loss of generality we may assume that $\Phi(G)\varsubsetneq N$, otherwise the result would be a direct consequence of the previous problem. Pick a maximal subgroup $K\subgroup N$ such that $\Phi(G)\subseteq K$. Put $\bar K=K/\Phi(G)$ and $\bar N=N/\Phi(G)$. By Problem~\ref{problem-1.D.6}, $\bar K\normal\bar N$ and $p=|\bar N:\bar K|$ is prime. Therefore, $K\normal N$ and $|N:K|=p$ is prime. In particular, $N/K$ is cyclic, hence abelian. Moreover, since $\bar K$ is nilpotent [cf.~Corollary~\ref{nilpotent-subgroups-and-quotients}], by induction on $|N|$ we may assume that $K$ is nilpotent. Given that $K \normal N$ and $N/K$ is abelian, we can extend any central series for $K$ to a central series for $N$.
\end{solution}
...
\begin{probl}\label{problem-2.D.16}
    ...
    \textrm{\upshape Hint. If some maximal subgroup $M$ fails to contain $\Phi(N)$, then $\Phi(N)M= G$, and it follows that $N = \Phi(N)(N\cap M)$.}
\end{probl}
...
\begin{probl}
    ...
    \textrm{\upshape Hint. The derived subgroup $N'$ is contained in $\Phi(N)$ by Problem~\ref{problem-1.D.8}.}

    \textrm{\upshape {\bf Note.} If we weakened the assumption that $G/N'$ is nilpotent and assumed instead that $G/N$ is nilpotent, it would not follow that $G$ is necessarily nilpotent.}
\end{probl}
...
\begin{probl}\label{problem-1.D.19}
    ...
    \textrm{\upshape Hint. Observe that $F(C)\normal G$.}
\end{probl}
...
\begin{xmpl}
    ...
    \begin{enumerate}[\upshape 1.]
        ...
        \item The remaining elements of order $2$ have the pattern $(xy)(\cdot\;\cdot)$, where the second transposition is determined by the first by complement. There are $\binom42/2=3$ of them.
        \item From facts {\upshape1} and {\upshape2}, there are $9$ elements of order $2$.
        ...
        \item From facts {\upshape4} and {\upshape5}, there are $8$ elements of order $3$.
        \item There are $3!=6$ elements of order $4$, which are cycles.
        \item If $P\in\Syl_2(S_4)$, then $\abs P=8$. It follows from fact~{\upshape3} that $n_2>1$.
        \item Since $n_2\mid 3$, it follows from fact~{\upshape8} that $n_2=3$.
        \item If $Q\in\Syl_3(S_4)$, then $\abs Q=3$. By fact~{\upshape6}, we deduce that $n_3>1$.
        \item Every subgroup of order $3$ must follow the pattern $\set{1,x,x^{-1}}$, for some\/ $x$ of order\/~$3$. By fact~{\upshape6}, there are\/ $8$ possibilities for\/ $x$ and\/ $x^{-1}$, which implies that\/ $n_3=4$.
...
\begin{proof} {\upshape[cf.~Exercise~\ref{exr:index-2-is-normal}]} Let $H$ be a subgroup of $G$ with $\abs{G:H}=2$. To show that $H$ is normal it suffices to show that $H^\omega\subseteq H$ for any $\omega\in G\setminus H$. Since $\omega H\ne H$, we may write $G=H\cup\omega H$. Now suppose that there is an element $\zeta\in H$ such that $\zeta^\omega\notin H$. Then
$$
...
\begin{xmpl}\label{A5-elements}
    ...
    \begin{enumerate}[-]
        \item {\upshape identity}
        \item {\upshape $3$-cycles:} there are $20$: $5\cdot4\cdot3$ with the pattern $(xyz)$, divided by $3$ because $(xyz)=(zxy)=(yzx)$.
        \item {\upshape $5$-cycles:} there are $4!=24$ of these.
        \item {\upshape disjoint $2$-cycles:} there are $15$: $\binom52\binom32$ with the pattern $(xy)(zw)$ divided by $2$ because $(xy)(zw)=(zw)(xy)$.
    \end{enumerate}
\end{xmpl}
...
Suppose toward a contradiction that $A_5$ has a proper normal subgroup $N$. Put $n=\abs N$. Note that
$$
...
\begin{proof} Suppose that $G$ is a nonabelian simple group with $n=\abs G<60$. Consider that the following facts lead to contradiction:
\begin{enumerate}[\upshape 1.]
    ...
    \item By fact {\upshape1}, $m$ is not a power of $2$, i.e., $m\in\set{3, 5, 6, 7, 9, 10, 11, 12, 13, 14}$.
    \item By fact {\upshape3}, $m$ is not prime, i.e., $m\in\set{6,9,10,12}$.
    \item By Theorem \ref{thm-p^3q}, $m$ is not $2\cdot p$, with $p$ prime, i.e., $m\in\set{9,12}$.
    \item By fact {\upshape8}, $n\in\set{2^2\cdot 3^2,\;2^4\cdot3}$.
    ...
    \item If $n=2^4\cdot3$, take $P\in\Syl_2(G)$. Then $\abs{G:P}=3$ and the same theorem as in fact~{\upshape10} shows the impossibility of this case.
\end{enumerate}
 \end{proof}
...
\begin{enumerate}[1.]
    ...
    \item If $\abs H=1$, all Sylow $p$ subgroups intersect trivially. By fact~1, there are $q(p^e-1)$ nontrivial elements whose orders are powers of $p$. The remanding $q$ elements have orders divisible by $q$. Therefore, there can only be one element in $\Syl_q(G)$, namely the one that consists of the $q$ elements whose order divides~$q$. Such a $q$-subgroup is normal, and we are done.
...
\begin{probl}
    ...
    \textrm{\upshape\textbf{Note.} If $|G|=36$, then a Sylow $3$-subgroup really can fail to be normal, but in that case, one can show that a Sylow $2$-subgroup of $G$ is normal.}
\end{probl}
...
\begin{probl}
    ...
    \textrm{\upshape Hint. Otherwise, show by counting elements that a Sylow $q$-subgroup must be normal and consider the factor group, of order $pr$.}

    \textrm{\upshape\textbf{Note.} In general, if $G$ is a product of distinct primes, then a Sylow subgroup for the largest prime divisor of $G$ is normal. We prove this theorem of Burnside when we study transfer theory in Chapter 5.}
\end{probl}
...
\begin{probl}
    ...
    \textrm{\upshape\textbf{Note.} This, of course, is also a consequence of the Feit-Thompson odd-order theorem, which asserts that no group of odd non-prime order can be simple.}
\end{probl}
...
\begin{enumerate}[\upshape1.]
    ...
    \item Then $|Q|=3^2$, meaning that $Q\in\Syl_3(K)$. Then $\Syl_3(K)=\set Q$, which contradicts the fact that $\Syl_3(K)$ had at least two elements $Q_1$ and $Q_2$.
\end{enumerate}
\end{solution}
...
\begin{probl}
    ...
    \textrm{\upshape Hint. If $G$ is simple, compute $n_7(G)$ and use Problem~\ref{problem-1.C.5}}
\end{probl}
...
\begin{enumerate}[\upshape1.]
    ...
    \item Since $A$ is normal, $\sigma$ is mono and we are assuming that $G$ is simple, it follows that $A=G$, i.e., $\sigma$ can be coastricted to $\sigma\colon G\to\Alt(S)$.
...
Take $p\in\spec(\ord(y))$. Since $\ord(y^p)=\ord(y)/p<\ord(y)$, we must have $y^p\in C$. Thus, $\gen{y^p}$ is a subgroup of $C$ of order $\ord(y)/p=n/(pm)$, hence generated by $c^{pm}$ [Corollary~\ref{cyclic-subgroups}]. It follows that $y^p=c^{pr}$ for some integer $r$. Therefore, $(yc^{-pr})^p=1$ with $yc^{-pr}\in G\setminus C$. By the definition of $y$ this implies that $\ord(y)=p$. Since $y\notin C$, it follows that
$C\cap\gen y=\gen1$.
...
\begin{thm}
    Let\/ $G$ be an abelian group. Then the following statements are equivalent:
    \begin{enumerate}[\upshape a)]
      \item $G$ is cyclic.
...
\begin{rem}\label{omega-props}${}$
    ...
    \end{enumerate}
    \textrm{\upshape Part e) follows from part c) in the case $G$ cyclic.}
\end{rem}
...
\end{equation}
For the first equation put $i=1$ and note that if $A=\gen a$ is a cyclic $p$-group, then $(a^j)^p=1$ if, and only if, $a^j\in\gen{a^{\ord(a)/p}}$. Thus, $|\Omega_1(A)|=p$.
...
\begin{align*}
    n_{i+1} &= \set{j\mid \Omega_i(A_j)\ne\Omega_{i+1}(A_j)}\\
        &= |\set{j\mid|A_j|\ge p^{i+1}}| &&\text{;\ \ref{omega-props}~e)}
\end{align*}
and so,
$$
...
\begin{thm}
    Let\/ $G$ be a finite abelian group, $G\cong \Z_{p_1^{e_1}}\times\cdots\times\Z_{p_k^{e_k}}$, with\/ $k\ge 0$, distinct primes\/ $p_1,\dots,p_k$ and\/ $e_i\ge1$. Then\/ $G$ is
    \begin{enumerate}[\upshape a)]
...
\begin{exr}
    Give an example of a nonabelian group that satisfies the statement of\/ \textrm{\upshape Theorem~\ref{subgroup-of-compatible-order}}.
\end{exr}
...
\begin{exr}
    With the help of\/ \textrm{\upshape Exercise~\ref{exercise-2.1.6}}, show Wilson's Theorem:
    $$
        (p - 1)! \equiv -1 \pmod p,
    $$
    where\/ $p$ is a prime.
...
\begin{solution} Apply the exercise to $G=\Z_p^*$. Since $x^2=1$ has two solutions, namely $x=1$ and $x=p-1$, we deduce that
$$
    p-1=\prod\Z_p^*=(p-1)!
$$
in $\Z_p$.  \end{solution}
...
\begin{cor}\label{brodkey-thm}{\upshape[Brodkey Theorem]}
    If every Sylow $p$-subgroup of the group\/~$G$ is abelian, then there exist $P,\,Q\in\Syl_p(G)$ such that $P\cap Q = O_p(G)$.
\end{cor}
...
\begin{probl}\label{problem-1.F.1}
    ...
    \textrm{\upshape Hint. Consider elements of the form $[x,y]$, where $x\in N$ and $y\in H$.}
\end{probl}
...
\begin{probl}\label{problem-1.F.3}
    ...
    \textrm{\upshape Hint. In fact, if $x\in N$ is chosen so that $|P\cap P^x|$ is as small as possible, then $P$ acts faithfully on the $P$-orbit containing $x$.}
\end{probl}
...
$$
with equality attained if, and only if, $H=C_G(C_G(H))$.  \end{proof} 
...
\begin{proof}
    With the notations of the theorem, let $M=\bigcap\cal L$. Since $\cal L$ is a lattice, $M\in\cal L$ and is minimal in this collection. It is abelian because $M$, being included in all members of $\cal L$, is also included in $C_G(M)$, which belongs to $\cal L$ according to the theorem. Finally, $Z(G)\subseteq C_G(C_G(M))=M$.  
\end{proof}
...
\begin{cor}\label{chermak-delgado} {\upshape[Chermak-Delgado]}
...
\begin{proof}
    Let $M=\CD(G)$ be the Chermak-Delgado subgroup of $G$. Then $M$ is characteristic because automorphisms preserve the Chermak-Delgado measure. By the previous corollary, $M$ is abelian. Moreover, if $A\subseteq G$ is abelian, then $A\subseteq C_G(A)$ and so $m_G(M)\ge m_G(A)=|A||C_G(A)|\ge |A|^2$, by Corollary~\ref{m_G-is-max}. Hence,
    $$
...
\begin{probl}
    ...
    \textrm{\upshape Hint. If $G$ is nonabelian, show that $H \in{\cal L}(G)$}.
\end{probl}
