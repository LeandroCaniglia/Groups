...
\begin{lem}\label{group-commutativity}{\upshape[Group Commutativity]}
...
\begin{defns} {\upshape[Symmetric group]}
    If $X$ is a nonempty set, the \textsl{symmetric group} $\Sym(X)$ is the set of all \textsl{permutations} of $X$, i.e., all bijections from $X$ to $X$ with the composition operation. When $X=\nset n$, the symmetric group is denoted by $S_n$. A \textsl{transposition} in $\Sym(X)$ is a permutation that interchanges two elements and leaves all other elements fixed.
\end{defns}
...
\begin{proof} Let $\sigma$ be a permutation. Pick $x\in X$. If $\sigma(x)=x$, then $\sigma$ induces a permutation $\bar\sigma$ in $X\setminus\set x$. It follows by induction on the cardinality of $X$ that $\bar\sigma$, and hence $\sigma$, is a product of transpositions. If $y=\sigma(x)\ne x$, let $\tau$ be the transposition that interchanges $x$ with $y$. Then $x$ is a fixed point of $\tau\sigma$. Again, the inductive hypothesis implies that $\tau\sigma$ is a product of transpositions. Then $\sigma=\tau(\tau\sigma)$ is a product of transpositions.
...
$$
    \Delta=(x_1-x_2)
        \prod_{2<k}(x_1-x_k)
        \prod_{2<h}(x_2-x_h)\cdot\delta(x_3,\dots,x_n),
$$
where $\delta$ includes the possible change of sign incurred by re-indexing. Thus,
$$
    \Delta_\tau=(x_2-x_1)
        \prod_{2<k}(x_2-x_k)
        \prod_{2<h}(x_1-x_h)\cdot\delta(x_3,\dots,x_n),
$$
which implies $(\ref{eq1})$.

After renaming indices as dictated by $\sigma$, equation $(\ref{eq1})$ can be rewritten as
$$
...
\begin{rem}
    ...
    \begin{quote}\small
        ...
        As the transposition adds $\pm1$ and all others add $0\pmod2$, a transposition changes the parity of the number of inversions.
    \end{quote}
\end{rem}
...
\end{align*}
which is a group epimorphism. In particular, its kernel $\Alt(X)$ is a (normal) subgroup of $\Sym(X)$ of order $n!/2$ that is called the \textsl{alternating group} on $X$. The alternating group on $\nset n$ is denoted by~$A_n$.

\begin{xmpl} Consider the alternating group $A_4$.
\begin{enumerate}[$\mbf\rightarrowtriangle$]
    \item It has $4!/2=12$ elements
    
    \item Its elements of order $3$ (which are exactly the $3$-cycles) are:
        \begin{align*}
            (123),\;(132),\;
            (124),\;(142),\;
            (143),\;(134),\;
...
$$
Since $\omega\mapsto(x\omega^{-1},\omega y)$ is injective, we get $\abs I\le|\mu^{-1}(z)|$. To establish the reverse inequality take $(a,b)\in\mu^{-1}(z)$. Then $ab=z=xy$. So, $\omega=by^{-1}$ is an element of $K$ and
$$
...
\begin{defns} Let $G$ be a group.
    \begin{enumerate}[\upshape i)]
        \item If\/ $X$ is a subset of\/ $G$, the \textsl{group generated by $X$}, denoted $\gen X$, is the intersection of all subgroups of $G$ containing $X$. For $X=\set{x}$ we simply write~$\gen x$. Note that $\gen x=\set{x^k\mid k\in\Z}$.
...
\begin{prop}{\upshape[Dedekind]}\label{dedekind}
...
\begin{prop}\label{frattini} Let\/ $G$ be a finite group.
    \begin{enumerate}[\upshape a)]
        ...
        \item If\/ $y\in G$ satisfies $\gen X\varsubsetneq G\Rightarrow\gen{X\cup\set y}\varsubsetneq G$ for all $X$, then $y\in\Phi(G)$.
...
\begin{defn}\label{right-index}
    If\/ $H$ is a subgroup of a group $G$, a \textsl{right} \textsl{coset} of $H$ is a set $Hx=\set{yx\mid y\in H}$, where $x\in G$. The number of right cosets, which may be infinite, is the \textsl{index} of\/ $H$ in $G$, and is denoted by $\abs{G:H}$.
\end{defn}
...
\begin{thm}\label{lagrange} {\upshape[Lagrange]}
...
\begin{rem}\label{left-index}
    The theorem, when applied to $G^{\textrm{op}}$, i.e., the set $G$ equipped with the opposite operation, automatically implies that conclusions~a), b), and~c) also hold for left cosets. In particular, the number of left cosets equals the index as defined above. 
\end{rem}
...
\begin{proof} {[cf.~Corollary~\ref{p-squared-is-abelian}]} If the group is cyclic, it is abelian. Otherwise, its non-identity elements have order $3$. Let $x$ be one of them and $y\notin\gen x$. If $xy=yx$, we are done. Otherwise $yx\in\set{xy^2, x^2y,x^2y^2}$.
\begin{enumerate}[$\to$]
...
\begin{rem}\label{rem:bijective-coclasses}
    A more conceptual proof of the corollary can be done when $K$ is normal in $H$ because, in that case, the following commutative diagram is exact:
    \small
    $$
        \begin{tikzcd}
    ...
    \begin{enumerate}[-]
        ...
        $$
            \bar\varphi(\bar x)=\bar\varphi(\bar z)
                \iff\varphi(x)\equiv\varphi(z)
                \iff x\equiv_\varphi z
                \iff \bar x=\bar z.
        $$
        \item In particular, if $\varphi$ is surjective, then $\bar\varphi$ is bijective.
...
\begin{cor}\label{coprime-indices}
...
\begin{proof} Put $G=\gen x$. If\/ $\sigma\in\text{Aut}(G)$, then there exists $k\ne0$ such that
$$
    \sigma(x)=x^k.
$$
Let $\eta$ be another automorphism. If $\eta(x)=x^h$, then $\sigma\eta(x)=x^{hk}=\eta\sigma(x)$, i.e., $\sigma\eta=\eta\sigma$.

In the case where $G$ is finite, with $\abs G=n$, we must have $k\perp n$, otherwise $\abs{\sigma(G)} < n$. Since we can also assume $1<k<n$ the result follows because
$$
...
\begin{rem}\label{characteristic-transitivity}
    ...
    $$
    \textrm{\upshape Indeed. If $\sigma\in\Aut(G)$, we can first define $\sigma_H\in\Aut(H)$, with $\sigma_H\subseteq\sigma$, and then $\sigma_K\in\Aut(K)$, with $\sigma_K\subseteq\sigma_H$.}
\end{rem}
...
\begin{prop}\label{normal-closure} Let $G$ be a group.
    \begin{enumerate}[\upshape a)]
        ...
        \item Let $x\in G$ and let $\gen x^G$ denote the subgroup generated by $\set{x^\omega\mid\omega\in G}$. Then $\gen x^G$ is normal.
    \end{enumerate}
\end{prop}
...
\begin{cor}\label{product-by-normal}
    ...
    $. In particular, if $X$ is a subgroup of\/ $G$, $XN$ is also a subgroup.
\end{cor}
...
\begin{rem}\label{biunivocal-normal-quotient}
    Given\/ $N\normal G$, the mapping\/ $H\mapsto H/N$ defines a bijection between the subgroups of\/ $G$ that contain\/ $N$ and the subgroups of\/~$G/N$.
\end{rem}
...
\begin{defn}
    ...
    $$
        C_G(X) = \set{z\in G\mid z\comm x \textrm{\upshape\ for all }x\in X}.
    $$
...
$$
    \ker(\sigma) = \set{\omega\in N\mid y^\omega = y\textrm{\upshape\ for all }y\in H}
        = C_N(H) = C
$$
...
\begin{defns}{\upshape[Commutator]}
...
For any $r \in \N$ and $1\le j\le r$, introduce the cyclic increment
$$
...
\begin{rem}
    Note that two conjugate permutations\/ $\alpha$ and\/ $\beta$ can always be conjugated by means of a permutation whose support is contained in the union of the supports of\/ $\alpha$ and\/ $\beta$.
\end{rem}
...
\begin{proof} The \textit{only if\/} part is trivial because $[\alpha,\beta]=\beta^\alpha\beta^{-1}$, which is the product of two permutations with the same cyclic structure. For the \textit{if\/} part, suppose that $\alpha$ and $\beta$ share the same cycle structure. Then $\alpha^{-1}$ and $\beta$ also have the same cycle structure. By the previous lemma, there exists $\sigma$ such that $\beta=(\alpha^{-1})^\sigma$. Therefore, 
$$
...
\begin{proof} {[\href{https://math.stackexchange.com/a/4825323/269050}{Brought from MSE}]} Every element of $A_n$ has a disjoint cycle decomposition that consists of cycles of odd lengths and an even number of cycles of even lengths [cf.~Remark~\ref{rem:cycle-decomposition-and-sign}]. So the result will follow if we show that cycles of odd length and products of two disjoint cycles of even length are commutators of elements whose support is contained in the support of the cycles.
...
\begin{defns}${}$
    \begin{enumerate}[\upshape-]
        ...
        $$
            \phi(y\cdot x) = y\cdot \phi(x)\quad\text{\upshape for all }x\in G,\, y\in L.
        $$
    \end{enumerate}
\end{defns}
...
\begin{rems}\label{derived-group}${}$
    \begin{enumerate}[\upshape i)]
        ...
        \item $G'=\bigcap\cal A$, where ${\cal A}=\set{N\normal G\mid G/N \text{ \upshape is abelian}}$.
    \end{enumerate}
\end{rems}
...
\begin{prop}\label{commutator-props}${}$ Let\/ $G$ be a group and\/ $x,y,z$ elements in\/ $G$. Then
    \begin{enumerate}[\upshape a)]
        \item $[zx,y] = [x,y]^z[z,y]$
        \item $[x,zy] = [x,z][x,y]^z$
        \item If\/ $X,Y$ are subgroups of\/ $G$ then\/ $[X,Y] \subseteq Y \iff Y \text{\upshape\ is }X$-invariant.
...
\begin{thm}\label{direct-product}
    ...
    \begin{equation}\label{eq5}
        (N_1\cdots N_{k-1})\cap N_k=\gen1\quad\textrm{\upshape for all }1<k\le r.  
    \end{equation}
\end{thm}
...
\begin{proof} This is a direct consequence of the previous corollary. Alternatively, let $n$ be the maximum order attained in $F^*$. Write $d=|F^*|$. We have to show that $n=d$. The last part of the previous lemma means that every element in $F^*$ is a root of the polynomial $f(x)=x^n-1$. Thus, $f$ has $d$ roots in $F^*$. Therefore, $n=\deg(f)\ge d$. The conclusion follows because $n\mid d$.  \end{proof}
    ...
    \begin{align*}
        [x_1x_2,y_1y_2] &= x_1x_2y_1y_2x_2^{-1}x_1^{-1}y_2^{-1}y_1^{-1}\\
            ...
            &= x_1y_1x_1^{-1}y_1^{-1}[x_2,y_2]
                &&;\ [x_2,y_2]\in N_2\text{ and }N_1\comm N_2\\
            &= [x_1,y_1][x_2,y_2].
    \end{align*}
...
\begin{exr}\label{exercise-1.1.2}
    ...
    \textrm{\upshape\textbf{Note.} A \textsl{traversal\/} of a subgroup $L$ in $G$ is a subset of $G$ that contains exactly one element of each left coset of $L$.}
\end{exr}

\begin{solution}
    First observe that, by definition, $m=|K:H|$ and $n=|G:K|$. Therefore, $nm=|G:K||K:H|=|G:H|$. We need to show that the $nm$ left cosets $x_iy_jH$ are all distinct. Suppose that $x_iy_jH = x_{i'}y_{j'}H$. Then $x_{i'}^{-1}x_iy_jH = y_{j'}H$. Since $y_j, y_{j'} \in K$ and $H \subseteq K$, both $y_jH$ and $y_{j'}H$ are subsets of $K$. This forces $x_{i'}^{-1}x_i \in K$, which implies $x_iK = x_{i'}K$. Because $\set{x_1, \dots, x_n}$ is a traversal for $K$ in $G$, we must have $i = i'$. It follows that $y_jH = y_{j'}H$, and since $\set{y_1, \dots, y_m}$ is a traversal for $H$ in $K$, we get $j = j'$. Since there are $|G:H|$ distinct cosets $x_iy_jH$, they form a traversal for $H$ in~$G$.
\end{solution}
...
\begin{solution} Since the \textit{if\/} part is trivial, we only need to prove the \textit{only if\/} part. Assume that $H\cup K$ is a subgroup and suppose there exists $y\in H\setminus K$. Take $x\in K$. Then $xy\in H\cup K$. But $xy\notin K$ because that would imply $y\in K$. Thus, $xy\in H$, which implies $x\in H$. Since $x$ was arbitrarily taken, $K\subseteq H$.
\end{solution}
...
\begin{solution} By hypothesis, $\omega^{-1}=yz^\omega$ for some $y,z\in H$. Then $1=y\omega z$, which implies $\omega = y^{-1}z^{-1} \in H$. Finally, $G=HH^\omega=HH=H$.
\end{solution}
...
\begin{exr}\label{exercise-1.1.7}
    ...
    \textrm{\upshape\textbf{Note.} An \textsl{involution\/} is an element of order~$2$.}
\end{exr}
...
\begin{solution} If the group is cyclic, it is $\Z_4$. Otherwise it has a subgroup $H$ of order $2$, which is necessarily (isomorphic to) $\Z_2$. Moreover, $H$ has index~$2$ and it is normal by the previous exercise. The quotient $G/H$ is isomorphic to $\Z_2$ too. Take $H=\gen y$ and $z\in G$ with $\gen{\bar z}=G/H$. Define
\begin{align*}
...
\begin{exr}\label{exr:simple-group-mention}
    ...
    \textrm{\upshape\textbf{Note.} A group is \textsl{simple\/} when it has no normal subgroup other than $\gen1$ and itself.}
\end{exr}

\begin{solution}
    Let $\varphi\colon H\to H/N$ be the canonical projection onto the quotient. Consider the composition $\varphi\circ\phi$. Since $G$ is simple $\varphi\circ\phi$ is trivial or mono. In the former case, $\phi(G)\subseteq N$, as wanted. In the latter case, the composition is a monomorphism from $G$ into $H/N$, which has order $2$. Since $G$ is simple, $|G| \ne 1$, meaning $G$ would be isomorphic to $H/N$ and $|G|=2$, which contradicts the hypothesis.
\end{solution}

\begin{exr}
    Let\/ $\omega \in G$, $C = \set{\omega^\zeta \mid\zeta\in G}$, and\/ $H_1, H_2\subgroup G$. Suppose that
    $$
        \gen C = G\quad\text{\upshape and}\quad C \subseteq H_1 \cup H_2.
    $$
    Then either\/ $H_1 = G$ or\/ $H_2 = G$.

    \textrm{\upshape Hint [l.c.]: Define $C_1=C\setminus H_2$ and show that $C_1=\emptyset$ or $\gen{C_1}\normal G$.}
\end{exr}
...
\begin{exr}
    ...
    \textrm{\upshape Hint [l.c.]: First observe that every pair of maximal subgroups of $G$ intersect trivially. Then use Exercises~\ref{exercise-1.1.11} and \ref{exercise-1.1.12}.}
\end{exr}
