...
Recall that $\Sym(X)$ denotes the group of bijections from $X$ to~$X$. Fix $n\ge2$. Let $\pi_k\colon X^n\to X$ denote the $k$th projection. Given a bijection $R=(\alpha,\beta)$ in $\Sym(X\times X)$ and two indices $1\le i\ne j\le n$, define $R^{ij}\in\Sym(X^n)$ as
$$
...
\begin{prop}
    ...
    \end{equation}
    if, and only if\/ $R$ is a solution to the quantum Yang-Baxter equation\/~\eqref{eq:qybe}.
\end{prop}

\begin{proof}
    \begin{align*}
        R^{12}R^{13}R^{23} &= (\alpha\pi_{12},\beta\pi_{12},\pi_3)
    ...
    \end{align*}
    Therefore, \eqref{eq:qybe} is equivalent to
    \begin{align}\label{eq:qybe-no-flip}
        ...
        \beta(\beta\pi_{12},\beta(\alpha\pi_{12},\pi_3))
            &= \beta(\pi_1,\beta\pi_{23}).\nonumber
    \end{align}
    On the other hand, since $\tau R=(\beta,\alpha)$, we can apply \eqref{eq:R^ij-i<j} and get
    \begin{align*}
    ...
    \begin{align*}
        ...
        r^{23}r^{12}r^{23} &= (\pi_1,\beta\pi_{23},\alpha\pi_{23})
            ...
            &= (\beta(\pi_1,\beta\pi_{23})
                ...
                ,\alpha(\alpha(\pi_1,\beta\pi_{23}),\alpha\pi_{23}))
    \end{align*}
    Hence, \eqref{eq:ybe} is equivalent to
    \begin{align}\label{eq:qybe-with-flip}
        ...
        \alpha(\alpha(\pi_1,\beta\pi_{23}),\alpha\pi_{23})
            &= \alpha(\alpha\pi_{12},\pi_3).\nonumber
    \end{align}
    The conclusion is now clear because \eqref{eq:qybe-no-flip} and \eqref{eq:qybe-with-flip} are the same.
\end{proof}
...
\begin{rem}
    ...
    \begin{equation}\label{eq:simpler-ybe}
        (r\times\id)({\id}\times r)(r\times\id)
            ...
            \tag{\textsc{ybe}}
    \end{equation}
    which does not require \eqref{eq:R^ij-i<j}.
\end{rem}
...
\begin{xmpl}
    ...
    $$
        ...
        \end{tikzcd}
    $$
    Suppose that $r$ solves~\eqref{eq:simpler-ybe} and let us see what we can say about $\sigma\cdot r$ for $\sigma\in\Sym(\Z_{n^2})$.
...
\paragraph{Structure and Permutation Groups.} Given\/ $X$ and\/ $r=(f,g)$ as above, the \textsl{structure group}\/ $G(X,r)$ is the group generated by\/ $X$ subject to the relations
$$
...
\begin{rem}
    ...
    $$
    Using translations, equation \eqref{eq:LB-distribution} can be written as
    $$
...
\begin{rem}
    Recall from Remark~\ref{rem:translated-group} that, for any element $x$ in a group $G$, the operation
    $$
        a\cdot_x b = ax^{-1}b
    $$
    turns $(G,\cdot_x)$ into a group, where $x$ serves as the identity and the inverse of~$a$ is~$xa^{-1}x$.

    With this notation \eqref{eq:SLB-distribution} can be written as
...
\begin{enumerate}[a),font=\upshape]
    \item If $n$ is even define the following operation in $\Z_n$
    $$
        a\skw b = (-1)^ba + b.
    $$
    ...
    \textbf{Note.} Incidentally, given that there is only one nonabelian group of $21$ elements, the two group structures must be isomorphic. Indeed, the map
    \begin{align*}
    ...
    \begin{enumerate}[a),font=\upshape]
        ...
        \begin{enumerate}[-]
            \item \textit{$\lambda_a$ is morphism:}
                ...
                \end{align*}
            \item \textit{$\lambda$ is a morphism:} $\lambda_a\circ\lambda_b=\lambda_{ab}$
                \begin{align*}
...
\begin{rems}${}$
    \begin{enumerate}[-]
        \item The map in \eqref{map:lambda} is usually written as
...
\begin{lem}\label{lem:ybe-theta}
    Let\/ $(G,\,\cdot\,,\skw)$ be a skew left brace and let\/ $(G,\skw)\rtimes(G,\,\cdot\,)$ be the semidirect product associated with the action\/ $\lambda$ defined in \text{\upshape Lemma~\ref{lem:ybe-lambda}}. Then the map
    \begin{align*}
...
\begin{proof}
    First note that the equation $b\skw\lambda_b(x)=b\cdot x$ implies that $b\skw I=b\cdot I$ for all $b\in B$ because $\lambda_b$ is bijective. In consequence, the underlying sets of $(B,\skw)/I$ and $(B,\,\cdot\,)/I$ are the same. Since these inherit the operations of the respective groups, in order to verify that $(B/I,\,\cdot_I\,,\,,\skw_I\,)$ is a SLB, we only need to verify the distributive property. Let $\varphi\colon B\to B/I$ be the set mapping defined by the projection onto the quotient. Then
    \begin{align*}
...
\begin{proof}${}$
    \begin{enumerate}[a),font=\upshape]
        ...
        First, observe that $b\mapsto\gamma_b^{-1}$ is a morphism because, according to Lemma \ref{lem:ybe-gamma}, $\gamma\colon(B,\,\cdot\,)^{\op{op}}\to\Sym(B)$ is a morphism and $\gamma_b^{-1}=\gamma_{b^{-1}}$. Thus, $\varphi$ is a morphism of groups and
        \begin{align*}
            b \in\ker\varphi
                &\iff bx=b\skw x
                    \text{ and }
...
\begin{ntn}
    Let\/ $(G,\,\cdot\,)$ and\/ $(N,\,\cdot\,)$ be groups and\/ $\sigma\colon G\to\Aut(N)$ an action of\/ $G$ on\/ $N$. Then\/ $Z^1(G,\sigma)$ denotes the set of\/ $1$-cocycles with respect to\/~$\sigma$, and\/ $\op{SLB}(G,\,\cdot\,)$ the set of operations {\upshape `$\skw$'} defined in\/ $G$ such that\/ $(G,\,\cdot\,,\,\skw\,)$ is a skew left brace.
\end{ntn}

\begin{thm}
    Bijective\/ $1$-cocycles are equivalent to skew left braces. More precisely, the map
    \begin{align*}
...
\begin{rem}\label{rem:Hol(G)}
    Every element $(x,\varphi)\in\Hol(G)$ can be seen as an endomorphism of $G$, namely
    $$
...
\begin{proof}
     To verify that the map is onto we have to show that, given $a\in G$, there is some $\phi\in\Aut(G)$ such that $(a,\phi)\in H$. By regularity there exists a pair $(b,\phi^{-1})\in H$ such that $b\phi^{-1}(a)=1$. Then
     $$
...
\begin{prop}\label{lem:iso-SLB}
    ...
    \end{equation}
    If\/ {\upshape`$\hat{\,\cdot\,}$'} is defined in the same way using another morphism\/ $\hat\lambda\colon G\to\Aut(G,\,\skw\,)$ that satisfies \eqref{eq:iso-SLB}, then\/ $(G,\,\cdot\,,\,\skw\,)$ and\/ $(G,\,\hat\cdot\,,\skw\,)$ are isomorphic if, and only if, there exists an automorphism\/ $\Gamma\in\Aut(G,\skw)$ such that 
...
\begin{proof}
    ...
    \begin{description}
        \item[\normalfont\textit{ if\/} part:] Let $\Gamma\in\Aut(G,\,\skw\,)$ be such that \eqref{eq:iso-SLB-lambda} does hold. Then,
        \begin{align*}
        ...
        \item[\normalfont\textit{only if\/}:] Suppose that $\Gamma\colon(G,\,\cdot\,,\,\skw\,)\to(G,\,\hat\cdot\,,\,\skw\,)$ is a SLB isomorphism. Then $\Gamma\in\Aut(G,\,\skw\,)$ and
        \begin{align*}
...
\begin{thm}
    ...
    \mathcal{B}(G) =\set{(G,\,\cdot\,,\,\skw\,)\mid
        (G,\,\cdot\,,\,\skw\,)\text{\upshape\ is a SLB}}
    $$
    and
    $$
    \mathcal{H}(G)=\set{H\mid H\text {\upshape\ is a regular subgroup of } \Hol(G)}.
    $$    
    Then the map
    \begin{align*}
